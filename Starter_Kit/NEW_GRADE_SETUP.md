...
6. In the hub file, edit the **`UNITS` array**: list the grade's units and topics. Set `status:'available'` + `file:'<module>.html'` for ready units; `status:'soon'` for the rest. Update the title text.
