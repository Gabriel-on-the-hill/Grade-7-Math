...
- **Anti-cheat/polish:** MC guess-cooldown (~1.6 s after a wrong click); answers decode via `ansVal()`/`optCorrect()`/`kdec()` with plain-attribute fallback (author plain, run the encode pass before shipping — *encode pass done 2026-07-02*); storage caps (struggles ≤120, responses ≤60 per topic); number inputs blur on scroll-wheel; keyboard-accessible tiles/cards. **Keyboard-operable controls (2026-07-20):** every interactive control must work without a pointer. The plot format shipped click-only and locked keyboard and screen-reader users out of six items, one a released MCAP capstone; it now carries `tabindex`, arrow-key stepping, `role="slider"` and `aria-valuenow`, guarded by `tests/plot_format.test.js`. **There is still no standing accessibility guard for the rest of the engine** — MC options, tiles and modals have never been tested for focus order, contrast or labelling. Treat that as an open gap (see `HANDOFF.md` S4), not a clean bill. **v1.4.2 additions (modules):** question-card text is unselectable and copy/cut events are suppressed (`user-select:none` on `.qcard`; inputs/textareas exempt) to blunt paste-into-a-search-engine cheating; MC/MS option order shuffles on every load for groups with **3+ options** (2-option groups like Yes/No or A-vs-B pairs keep their semantic order; add `data-noshuffle` to any group to opt out); exam capstones carry **no hints**; hints are strategy-only (§2.4).
...
- **2026-07-21** — **Hand-off execution: key audit, a grading-defect fix, the MO-6 habit layer, and three MCAP lifts.**
  Worked the [HANDOFF.md](HANDOFF.md) task list.
  *S1 — key audit.* Independently recomputed **all 181 item keys** across the five modules (script-dumped each key beside
  its stem; every arithmetic key correct). The pass surfaced a real defect: three **"select all" items authored as
  single-select `mc-group`s** with two correct options each — Geometry `2-2`, Geometry `3-4` (MCAP Q20), Ratios `r6-7` —
  which the engine completed on the *first* correct click, so "select all" could be satisfied by one pick. Two even carried
  an inert `data-multi="1"` no JS reads. Converted all three to `ms-group` (grades every option); added a
  `module_integrity` guard (**every single-select `mc-group` has exactly one correct option**), mutation-checked.
  *T3 — MO-6 habit layer.* A quiet, day-based **consistency streak** ("N sessions in a row") in the hub, beside — never
  inside — the mastery tiles: advances at most once per calendar day, only on real practice (truth floor), **forgives a
  single missed day**, display-only (never feeds mastery, `MO-7`). One-time dismissible "same time, same place" cue. Not
  the accuracy-earned per-topic `reviewStreak` (that resets on a wrong answer — punitive where `MO-6` says be forgiving).
  Guarded in the behavioural suite (forgiving-day, unfarmable, truth-floor, `MO-7` isolation), mutation-checked.
  `MO-2/3/4` (points/XP/badges) **declined** on evidence (access+habit, not motivation; no cohort) — `PEDAGOGY_ALIGNMENT`
  item 4 updated.
  *T2 + T1 — three real MCAP items.* Ratios `r3-7` = **MCAP Q1** (four-graph "which has k = ½" select; four option graphs
  rebuilt as inline SVG from the rendered page); Geometry `7-4` = **MCAP Q18** (ceramic-tile count, figure-free); and
  Expressions `4-3` **restored to MCAP** = **Modeling Q16** — the 20 Jul demotion ("not in any packet") was a text-search
  error; the item is Q16, and the one missing distractor was added for full fidelity. New trap recorded in
  `MCAP_PROVENANCE.md`: *a negative text search is not proof of absence — render the page before un-lifting an item.*
  Remaining T1 (documented in the hand-off): the other reasoning targets (Q15/Q31/Q33) are **drop-down** items the engine
  can only render as MC (adapted, not MCAP-liftable); Q30 is a figure-referenced "write an expression" CR.
  *T5.* Fareedah's ledger brought current from the June + July summaries (DRAFT cleared); the access-not-motivation
  diagnosis behind `D3` is corroborated (she needed help to reach the app every session through 17 Jul).
  Verified: all 10 guards green, behavioural suite **208 assertions**, every new/changed key recomputed, every new SVG
