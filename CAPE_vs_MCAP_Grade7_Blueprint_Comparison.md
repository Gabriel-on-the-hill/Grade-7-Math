...
| Dimension | **DC CAPE (DC Math)** | **Maryland MCAP** |
|---|---|---|
| Lineage / vendor | PARCC continuation; Pearson / TestNav | PARCC-descended, Maryland-specific |
| Organizing frame | **Task Type** (PARCC-style): Type 1 / 2 / 3 | **Subclaim**: Content / Reasoning / Modeling |
| Standards | Common Core Grade 7 (7.RP, 7.NS, 7.EE, 7.G, 7.SP) | **Same** Common Core Grade 7 domains |
| Operational items (Gr 7) | **31** | **35** |
| Total points (Gr 7) | **~52** | **~45** |
| Per-item point values | Type 1: **1, 2, or 4 pt**; Type 2: 3–4 pt; Type 3: **3 or 6 pt** | Content: **1 pt only**; Reasoning/Modeling CR: 3 pt & 4 pt |
| Item interactions | MC, MS, FIB, **TE** (technology-enhanced). No CR/human-scored mentioned | MC, MS, fill-in, TE **plus explicit human-scored constructed response** |
| Published domain weights? | **No** — high-level blueprint is task-type/points only | **Yes** — explicit per-domain item counts |
| Administration | Computer-based (TestNav); **3 sections × 60 min** (Gr 6–7) | Computer-based; timing not in this blueprint |
| Calculator (Gr 7) | Section 1 non-calc/calc, **Sections 2–3 calculator** | Calc/non-calc sections (per MCAP practice) |

### CAPE Grade 7 item structure (from the blueprint appendix)
- **Type 1 — concepts, skills, procedures:** 20 × 1-pt + 5 × 2-pt = **25 items / 30 pts**
- **Type 2 — mathematical reasoning:** 2 × 3-pt + 1 × 4-pt = **3 items / 10 pts**
- **Type 3 — modeling / application:** 2 × 3-pt + 1 × 6-pt = **3 items / 12 pts**
- **Total: 31 items / 52 points.** All tasks standalone.

### MCAP Grade 7 item structure (from the blueprint)
- **Content Subclaim — 23 items, all 1-pt, machine-scored**, weighted by domain:
  - Ratios & Proportional Relationships (7.RP.A): **8** (~35%)
  - Equations & Expressions (7.EE.A/B): **5** (~22%)
  - The Number System (7.NS.A): **4** (~17%)
  - Geometry (7.G.A/B): **3** (~13%)
  - Statistics & Probability (7.SP.A/B/C): **3** (~13%)
- **Reasoning Subclaim — 6 items** (4 × 1-pt machine + 1 × 3-pt CR + 1 × 4-pt CR); content limited to **RP, NS, EE** (7.R.1–3).
- **Modeling Subclaim — 6 items** (4 × 1-pt machine + 1 × 3-pt CR + 1 × 4-pt CR); any domain (7.M.1–6).
- **Total: 35 items / ~45 points.**
...
3. **Constructed response vs technology-enhanced.** MCAP explicitly includes **human-scored constructed-response** items in both Reasoning and Modeling (four CR items total). CAPE's blueprint lists only **MC / MS / FIB / TE** interactions — leaning toward machine/technology-enhanced scoring, with no explicit "written CR." Practically: CAPE reasoning/modeling is more likely a **multi-step TE task** than a "type your explanation" prompt.

4. **Published domain weighting.** MCAP *tells you* the domain emphasis (RP heaviest at 8; Geometry and Stats/Prob light at 3 each). CAPE's high-level blueprint **does not publish domain counts** — only task-type/point structure. The domain emphasis for CAPE is inferable from CCSS "major work of Grade 7" (RP, NS, EE emphasized; G, SP supporting) but is **not stated** in this document.
