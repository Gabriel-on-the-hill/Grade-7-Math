...
// §2.3 "Flow builds up": within a section the climb runs Learn → Guided → Practice → Apply → Exam.
// Stretch is deliberately unranked — it is beyond-the-standard bonus and may sit anywhere.
const RANK = { learn: 0, guided: 1, practice: 2, apply: 3, exam: 4 };

for (const f of MODULES) {
  const h = fs.readFileSync(path.join(DIR, f), 'utf8');
  const bad = [];

  // --- 5. the climb within each section must not run backwards ---------------------------------
  // Appending a card to the end of a section is the easy way to break this, and it is invisible in
  // a diff: a Guided card landing after three Practice ones, or an Apply after the Exam capstone.
  for (const sec of h.split(/<section class="topic" id="/).slice(1)) {
    const sid = (sec.match(/^([^"]+)"/) || [])[1] || '?';
    const body = sec.split(/<\/section>/)[0];
    const seq = [];
    for (const p of body.split(/<div class="qcard[\s"]/).slice(1)) {
      const qid = (p.match(/data-qid="([^"]+)"/) || [])[1];
      if (!qid || /\+String/.test(qid)) continue;
      const chip = (p.slice(0, 400).match(/<span class="qtag (\w+)"/) || [])[1];
      if (chip && chip in RANK) seq.push({ qid, chip, rank: RANK[chip] });
    }
    for (let i = 1; i < seq.length; i++) {
      if (seq[i].rank < seq[i - 1].rank) {
        bad.push('section "' + sid + '": climb runs backwards — ' + seq[i].qid + ' (' + seq[i].chip +
                 ') comes after ' + seq[i - 1].qid + ' (' + seq[i - 1].chip + ')');
      }
    }
  }
